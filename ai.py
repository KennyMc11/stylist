...
class AIStylist:
    ...
    async def generate_outfit(self, user_data: Dict, event: str, weather: Dict) -> str:
        ...
        Опиши образ коротко, но лаконично, что надеть, как сочетать (2-3 маленьких абзаца), без воды. Используй эмодзи. Используй HTML-разметку для форматирования текста, чтобы он корректно отображался в Telegram. Будь дружелюбным."""
    ...
    async def chat_with_stylist(self, user_data: Dict, message: str, context: List[Dict]) -> str:
        ...
        - Используй HTML-разметку для форматирования текста, чтобы он корректно отображался в Telegram."""
